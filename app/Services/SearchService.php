...
class SearchService
{
    ...
    private function searchLeads(string $q, User $user): array
    {
        $cols = ['first_name', 'last_name', 'email', 'phone', 'lead_id', 'tracking_code',
            'passport_number', 'inz_client_number', 'inz_application_number', 'inz_medical_ref', 'nzer_number'];
        ...
        return $this->collect($count, $cand, function (Lead $l) use ($user) {
            ...
            return $this->scoreFields([$name, $l->first_name, $l->last_name, $l->email, $l->phone, $l->lead_id, $l->tracking_code, $l->passport_number, $l->inz_client_number, $l->inz_application_number, $l->inz_medical_ref, $l->nzer_number], $q);
        });
    }
