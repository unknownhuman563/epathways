...
class EngagementDocumentGenerator
{
    ...
    {
        $docs = collect(self::DOCS)->map(fn ($d, $key) => [
            'key' => $key,
            'label' => $d['label'],
        ...
        // The tax invoice rides along in the same pack — generated by the
        // InvoiceGenerator (not this class) off the agreed fees, and it also
        // surfaces in the Invoice workspace like any other invoice. Sits right
        // after the Written Agreement (they share the fee figures).
        array_splice($docs, 1, 0, [[
            'key' => 'invoice',
            'label' => 'Invoice',
            'description' => 'Tax invoice for the agreed fees — also appears in the Invoice workspace.',
            'dynamic' => true,
        ]]);
    ...
    private function writtenAgreementData(Lead $lead, array $overrides): array
    {
        ...
        foreach ($applicants as $a) {
            $f = $this->applicantFee($a, $tier, $location);
            if ($f !== null) {
        ...
        if (! $overrideSet && count($applicants) > 1) {
            foreach ($applicants as $a) {
                $professionalLines[] = [
                    ...
                    'amount' => $this->money($this->applicantFee($a, $tier, $location)),
                ];
            }
        }
    ...
    private function buildApplicants(Lead $lead): array
    {
        ...
        $deps = [];
        // Only dependants staff have marked as part of this agreement. The flag
        // defaults on, so existing families are unaffected until a member is
        // explicitly excluded.
        foreach ($lead->dependents()->where('in_agreement', true)->with(['visaType', 'linkedLead'])->get() as $dep) {
    ...
    /**
     * The professional fee for one applicant (built by buildApplicants) at the
     * chosen tier/location — the per-applicant override when set, else the
     * visa's fee. Public so the invoice generator applies the same rule.
     */
    public function applicantFee(array $a, string $tier, string $location): ?float
    {
        if (isset($a['fee_override']) && $a['fee_override'] !== null) {
            return (float) $a['fee_override'];
        }
