...
// Stacked floating WhatsApp + Messenger buttons (desktop) — sits just above
// the existing ScrollToTop / ChatBot trigger at bottom-right. Hidden on
// admin/portal/auth surfaces because those aren't public landing pages.
//
// Buttons are conditionally rendered: if the underlying contact channel
// has no value in config('services.contact.*'), that icon is hidden so the
// stack collapses gracefully.
export default function FloatingContact() {
    ...
    const whatsappHref = contact?.whatsapp
        ...
        : null;
    const messengerHref = contact?.messenger || null;
        ...
        <div className="hidden md:flex fixed right-6 bottom-28 z-40 flex-col items-end gap-3 pointer-events-none">
            ...
            {messengerHref && (
                <a
                    ...
                    className="pointer-events-auto group relative w-14 h-14 rounded-full bg-[#0084FF] shadow-[0_15px_35px_-5px_rgba(0,0,0,0.4)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all"
                >
                    <svg viewBox="0 0 32 32" className="w-7 h-7" fill="white" aria-hidden="true">
                        <path d="M16 2C8.27 2 2 7.74 2 15.04c0 4.13 1.92 7.81 4.92 10.22V31l4.5-2.47c1.2.33 2.47.51 3.78.51 7.73 0 14-5.74 14-13.04S23.73 2 16 2zm1.48 17.4l-3.65-3.89-6.87 3.89 7.55-8.02 3.65 3.89 6.87-3.89-7.55 8.02z" />
                    </svg>
                    <span className="absolute right-full mr-3 px-3 py-1.5 rounded-full bg-[#282728] text-white text-[9px] font-bold uppercase tracking-[0.2em] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        Messenger
                    </span>
