...
// Day / Week / Month calendar for immigration consultation bookings — same
// interaction model as the Task Board calendar, but lays bookings onto the
// grid by their appointment_date and colours them by payment status. Clicking
// a booking (or a day) selects that date so the parent can filter its list.

const WEEKDAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

const CHIP = {
    paid: "bg-emerald-50 hover:bg-emerald-100 border-emerald-200 text-emerald-900",
    unpaid: "bg-amber-50 hover:bg-amber-100 border-amber-200 text-amber-900",
};
...
export default function BookingsCalendar({ bookings = [], selectedKey = null, onSelectDay }) {
    ...
    const pickDay = (d) => {
        const k = keyOf(d);
        onSelectDay?.(selectedKey === k ? null : k);
    };
                            ...
                            return (
                                <div key={k} onClick={() => pickDay(day)} className={`min-h-[128px] border-b border-gray-100 cursor-pointer ${idx % 7 === 0 ? "" : "border-l"} ${selected ? "bg-amber-50/60" : inMonth ? "bg-white hover:bg-gray-50/60" : "bg-gray-50/40"}`}>
                                    ...
                                    <div className="px-1.5 pb-1.5 space-y-1">
                                        {list.slice(0, 3).map((b) => <BookingChip key={b.id} b={b} />)}
                            ...
                            return (
                                <div key={k} onClick={() => pickDay(day)} className={`min-h-[380px] cursor-pointer ${idx === 0 ? "" : "border-l border-gray-100"} ${selected ? "bg-amber-50/60" : "bg-white hover:bg-gray-50/50"}`}>
                                    <div className="px-1.5 py-1.5 space-y-1">
                                        {list.length === 0 ? <p className="text-[10px] text-gray-300 text-center mt-4">No bookings</p> : list.map((b) => <BookingChip key={b.id} b={b} />)}
                                    </div>
                                </div>
                            );
                        })}
            ...
            {mode === "day" && (() => {
                ...
                return (
                    <div className="px-4 py-5">
                        {list.length === 0 ? (
                            <p className="text-[12px] text-gray-400 text-center py-10">No bookings on this day.</p>
                        ) : (
                            <div className="space-y-2 max-w-2xl mx-auto">{list.map((b) => <BookingChip key={b.id} b={b} wide />)}</div>
                        )}
                    </div>
                );
            })()}
...
function BookingChip({ b, wide = false }) {
    const paid = b.payment_status === "paid";
    return (
        <div
            title={`${b.name}${b.visa ? ` — ${b.visa}` : ""}`}
            className={`block w-full text-left rounded-lg border px-2 py-1.5 ${CHIP[paid ? "paid" : "unpaid"]} ${wide ? "px-3 py-2.5" : ""}`}
        >
            ...
            <div className="flex items-center gap-2 mt-0.5 text-[10px] opacity-80">
                ...
                <span className="font-semibold">{paid ? "Paid" : "Unpaid"}</span>
            </div>
            {b.visa && <p className="text-[10px] opacity-70 truncate mt-0.5">{b.visa}</p>}
            {wide && b.consultant_name && <p className="text-[10px] opacity-70 truncate mt-0.5 inline-flex items-center gap-1"><User size={9} /> {b.consultant_name}</p>}
        </div>
    );
}
