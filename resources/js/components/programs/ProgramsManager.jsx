...
import { GraduationCap, Search, MapPin, Building2, Users, ExternalLink, Plus, X, Save, Pencil, Trash2 } from "lucide-react";
...
// Add / Edit modal. `editing` null = add; object = edit (pre-filled, posts to
// the update route). The parent remounts it via `key` so initial values are
// always fresh.
function ProgramModal({ editing, portalBase, onClose }) {
    const isEdit = !!editing;
    const { data, setData, post, processing, errors, reset } = useForm({
        title: editing?.title || "",
        institution: editing?.institution || "",
        location: editing?.location || "",
        level: editing?.level || 7,
        category: editing?.category || "bachelors",
        status: editing?.status || "draft",
        price_text: editing?.price_text || "",
        duration_months: editing?.duration_months ?? "",
        intake_months: editing?.intake_months || "",
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 p-0 sm:p-6" onClick={(e) => { if (e.target === e.currentTarget && !processing) onClose(); }}>
            <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h2 className="text-base font-bold text-gray-900 leading-none">{isEdit ? "Edit program" : "Add program"}</h2>
                            <p className="text-[11px] text-gray-500 mt-0.5">{isEdit ? "Update this catalogue entry" : "Add a program to the shared catalogue"}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={submit} className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <Field label="Institution">
                            <input type="text" value={data.institution} onChange={(e) => setData("institution", e.target.value)} className={IC} maxLength={255} placeholder="e.g. University of Auckland" />
                        </Field>
                    ...
                    <Field label={isEdit ? "Replace banner image (optional)" : "Banner image"}>
                ...
                <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-gray-100 bg-gray-50">
                    ...
                    <button type="button" onClick={submit} disabled={processing} className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gray-900 text-white text-[12px] font-bold uppercase tracking-wider hover:bg-gray-800 transition-colors disabled:opacity-40">
                        <Save size={12} /> {processing ? "Saving…" : (isEdit ? "Save changes" : "Add program")}
                    </button>
                </div>
            </div>
        </div>
