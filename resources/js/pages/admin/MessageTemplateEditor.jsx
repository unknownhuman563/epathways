...
export default function MessageTemplateEditor({ template = null, standardVariables = [] }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (editing) form.put(`/admin/message-templates/${template.id}`, { preserveScroll: true });
        else form.post("/admin/message-templates");
    };
    ...
    const sendTest = () => {
        router.post(`/admin/message-templates/${template.id}/test`, { email: testEmail || null }, { preserveScroll: true });
    };
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            ...
            <Link href="/admin/message-templates" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            <label className="block">
                                ...
                                <input value={data.key} onChange={(e) => setData("key", e.target.value)} disabled={editing} placeholder="doc_approved" className={`${inp} ${editing ? "bg-gray-50 text-gray-500" : ""}`} />
